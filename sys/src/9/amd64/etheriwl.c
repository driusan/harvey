...
struct Ctlr {
	...
	struct {
		...
		unsigned char	step;
		unsigned char	dash;
		unsigned char	txantmask;
		unsigned char	rxantmask;
	} rfcfg;
...
static char *qcmd(Ctlr *ctlr, uint qid, uint code, unsigned char *data, int size, Block *block);
static char *flushq(Ctlr *ctlr, uint qid);
static char *cmd(Ctlr *ctlr, uint code, unsigned char *data, int size);
...
static void
put32(unsigned char *p, uint v){
	*((uint32_t*)p) = v;
}
static void
put16(unsigned char *p, uint v){
	*((uint16_t*)p) = v;
};
...
static void
setfwinfo(Ctlr *ctlr, unsigned char *d, int len)
{
	FWInfo *i;
...
static char*
crackfw(FWImage *i, unsigned char *data, uint size, int alt)
{
	unsigned char *p, *e;
	FWSect *s;
...
readfirmware(char *name)
{
	...
	unsigned char dirbuf[sizeof(Dir)+100], *data;
	char buf[128], *err;
	FWImage *fw;
	int n, r;
	Chan *c;
...
rbplant(Ctlr *ctlr, int i)
{
	...
	if(b == nil)
		return -1;
	b->rp = b->wp = (unsigned char*)ROUND((uintptr)b->base, 256);
...
postboot(Ctlr *ctlr)
{
	...
	for(q=0; q<7; q++){
		if(ctlr->type != Type4965){
			static unsigned char qid2fifo[] = { 3, 2, 1, 0, 7, 5, 6 };
			prphwrite(ctlr, SchedQueueStatus5000 + q*4, 0x00ff0018 | qid2fifo[q]);
		} else {
			static unsigned char qid2fifo[] = { 3, 2, 1, 0, 4, 5, 6 };
	...
	if(ctlr->type != Type4965){
		...
		if(ctlr->calib.done == 0){
			...
			static unsigned char cmds[] = {8, 9, 11, 17, 16};
...
static char*
loadfirmware1(Ctlr *ctlr, uint32_t dst, unsigned char *data, int size)
{
	unsigned char *dma;
	char *err;
...
static char*
qcmd(Ctlr *ctlr, uint qid, uint code, unsigned char *data, int size, Block *block)
{
        Proc *up = externup();
...
static char*
cmd(Ctlr *ctlr, uint code, unsigned char *data, int size)
{
	char *err;
...
static void
addnode(Ctlr *ctlr, unsigned char id, unsigned char *addr)
{
	unsigned char c[Tcmdsize], *p;
...
static void
iwlmulticast(void * unused, unsigned char* unused2, int unused3)
{
}
...
receive(Ctlr *ctlr)
{
	...
	for(hw = get16(rx->s) % Nrx; rx->i != hw; rx->i = (rx->i + 1) % Nrx){
		unsigned char type, flags, idx, qid;
		uint32_t len;
