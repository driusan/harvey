...
static unsigned char basicrates[] = {
	0x80 | 2,	/* 1.0	Mb/s */
	0x80 | 4,	/* 2.0	Mb/s */
	0x80 | 11,	/* 5.5	Mb/s */
	0x80 | 22,	/* 11.0	Mb/s */
...
static Wnode*
nodelookup(Wifi *wifi, unsigned char *bssid, int new)
{
	Wnode *wn, *nn;
...
static unsigned char*
putrates(unsigned char *p, unsigned char *rates)
{
	int n, m;
...
static void
recvassoc(Wifi *wifi, Wnode *wn, unsigned char *d, int len)
{
	uint s;
...
static void
recvbeacon(Wifi *wifi, Wnode *wn, unsigned char *d, int len)
{
	static unsigned char wpa1oui[4] = { 0x00, 0x50, 0xf2, 0x01 };
	unsigned char *e, *x, *p, t;
	int rsnset;
...
static int
hextob(char *s, char **sp, unsigned char *b, int n)
{
	int r;
...
{
	// static unsigned char zeros[Eaddrlen];
...
static void
tkipk2tk(unsigned char key[16], uint16_t tk[8])
{
...
static void
tkipphase1(uint32_t tscu, unsigned char ta[Eaddrlen], uint16_t tk[8], uint16_t p1k[5])
{
...
static void
tkipphase2(uint16_t tscl, uint16_t p1k[5], uint16_t tk[8], unsigned char rc4key[16])
{
...
static void
micsetup(MICstate *s, unsigned char key[8])
{
	s->l =	(uint32_t)key[0] |
		(uint32_t)key[1]<<8 |
		(uint32_t)key[2]<<16 |
...
static void
micupdate(MICstate *s, unsigned char *data, uint32_t len)
{
	uint32_t l, r, m, n, e;
...
static void
micfinish(MICstate *s, unsigned char mic[8])
{
	static unsigned char pad[8] = { 0x5a, 0x00,  0x00,  0x00,  0x00,  0x00,  0x00,  0x00, };
...
static unsigned char pad4[4] = { 0x00, 0x00, 0x00, 0x00, };
...
static unsigned char*
putbe(unsigned char *p, int L, uint v)
{
	while(--L >= 0)
		*p++ = (v >> L*8) & 0xFF;
	return p;
...
static void
xblock(int L, int M, unsigned char *N, unsigned char *a, int la, int lm, unsigned char t[16], AESstate *s)
{
	unsigned char l[8], *p, *x, *e;
...
static unsigned char*
sblock(int L, unsigned char *N, uint i, unsigned char b[16], AESstate *s)
{
	b[0] = L-1;	/* flags */
	memmove(&b[1], N, 15-L);
	putbe(&b[16-L], L, i);
...
static void
aesCCMencrypt(int L, int M, unsigned char *N /* N[15-L] */,
	unsigned char *a /* a[la] */, int la,
	unsigned char *m /* m[lm+M] */, int lm,
	AESstate *s)
{
	unsigned char t[16], b[16], *p, *x;
	uint i;
...
static int
aesCCMdecrypt(int L, int M, unsigned char *N /* N[15-L] */,
	unsigned char *a /* a[la] */, int la,
	unsigned char *m /* m[lm+M] */, int lm,
	AESstate *s)
{
...
static int
setupCCMP(Wifipkt *w, uint64_t tsc, unsigned char nonce[13], unsigned char auth[32])
{
	unsigned char *p;
...
{
	unsigned char auth[32], nonce[13];
